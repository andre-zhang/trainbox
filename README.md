...
- Import a city’s existing transit network from OpenStreetMap: pick the area, choose which modes to pull in, then merge into your map. Re-importing can open a review step when a new route looks too close to something you already drew, so you merge or skip per row instead of overwriting blindly.
